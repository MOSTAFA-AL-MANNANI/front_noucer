...
export default function FiliereCRUD() {
  ...
  const fetchFilieres = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: `${filieres.length} filière(s) chargée(s)`,
        icon: 'success',
        timer: 2000,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) {
      showAlert("Champ requis", "Veuillez saisir un nom pour la filière", "warning");
      return;
    }
    ...
    try {
      if (editId) {
        ...
        Swal.fire({
          ...
          text: `La filière "${name}" a été modifiée avec succès`,
          icon: 'success',
          confirmButtonColor: '#3085d6'
        });
        ...
        Swal.fire({
          ...
          text: `La filière "${name}" a été créée avec succès`,
          icon: 'success',
          confirmButtonColor: '#3085d6'
        });
      }
      ...
      showAlert("Erreur", "Erreur lors de l'opération sur la filière", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (filiere) => {
    ...
    Swal.fire({
      title: 'Modification',
      text: `Vous modifiez la filière "${filiere.name}"`,
      icon: 'info',
      timer: 1500,
      showConfirmButton: false,
      position: 'top-end',
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          title: 'Supprimé !',
          text: `La filière "${filiere.name}" a été supprimée`,
          icon: 'success',
          confirmButtonColor: '#3085d6'
        });
        ...
        showAlert("Erreur", "Erreur lors de la suppression. La filière est peut-être utilisée.", "error");
      }
    }
  };
